...
size_t Mark5BUnpacker::extract_samples(char const* const src, Ipp32f* dst, const size_t count, const int channel) const
{
    ...
    // Data format:
    // We assume it is 2-bit with either sign,mag or mag,sign ordering.
    // Similar to TwoBitUnpacker and the mapping is {m,s}
    ...
    /* unpack using the lookup table */
    unsigned char const* src8 = (unsigned char const*)src + channel/2; // byte offset
    int step = cfg->source_channels / 2; // 1 or 2
    for (smp=0; smp<count; ) {
        const int unroll_factor = 8;
        for (char off=0; off<unroll_factor; off++) {
            dst[smp+off] = precooked_LUT[*(src8+off*step)];
